...
_DB_PATH_=str(Path(__file__).parent.parent.parent/"data.db")

@st.cache_resource
def _get_connection():
    conn=sqlite3.connect(_DB_PATH_,check_same_thread=False)
    conn.row_factory=sqlite3.Row  #return data in terms of col name not indices
    return conn
    ...
    with conn:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS exercises (
                ...
                time          INTEGER NOT NULL DEFAULT 0,       -- how much time (in secs) spent in this exercise
                created_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
            """
        )
